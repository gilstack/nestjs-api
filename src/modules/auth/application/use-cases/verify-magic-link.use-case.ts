...
import type { VerifyMagicLinkDto, AuthResponseDto } from '../dtos';
...
export class VerifyMagicLinkUseCase {
    ...
    async execute(
        ...
        if (!user) {
            ...
            this.logger.info('New user created via magic link', { userId: user.id, email: normalizedEmail });
        }
        ...
        response.cookie('refresh_token', refreshTokenJwt, {
            httpOnly: true,
            secure: isSecure,
            sameSite: 'strict',
